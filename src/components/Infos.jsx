    ...
    <div className='flex inset-0 items-center fixed bg-black  bg-opacity-35 justify-center backdrop-blur-sm z-50 ' id='infos' onClick={close}>
      <div className='bg-white lg:max-w-md w-full h-[99%] lg:max-h-[78%] rounded-md overflow-hidden mt-2 overflow-y-scroll animaDiv'>
        ...
        <div className='px-4 mt-6'>
            <p className='flex gap-2 text-[18px] px-2 items-center border max-w-[140px] rounded-full  justify-center'><span className={`w-3 h-3 rounded-full ${hora < 16 ? "bg-red-600":"bg-green-600"}`}></span><span>{hora < 18 ? "Fechado":"Fechado"}</span> <FaClock className='text-gray-500'/> </p>
        </div>
...
<a href="https://api.whatsapp.com/send?phone=37998362445&text=ola, vim la do site" className=' border-b-2 pb-4 block'><FaWhatsapp className='text-3xl'/></a>
        </div>
